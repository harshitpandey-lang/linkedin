...
- Gemini uses the REST `v1beta/{resource}:generateContent` endpoint with the documented `x-goog-api-key` header. The default is stable `gemini-2.5-flash`; the retired Gemini 2.0 Flash identifier was removed from config and examples. The provider preserves Google resource names as `models/...`, accepts either configured model spelling without creating `models/models/...`, inspects `name`, `displayName`, and `supportedGenerationMethods`, excludes TTS/image/embedding/live fallbacks, and retries with the best stable text model after a 404. Diagnostics report both the configured and attempted resources without exposing the API key.
