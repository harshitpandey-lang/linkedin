...
- `SUPABASE_SERVICE_ROLE_KEY` is a server-only modern Supabase Secret Key in `sb_secret_...` format. The dependency is pinned to the current stable `supabase==2.31.0`; because that SDK release still has a legacy JWT-only local key check, `get_client()` bootstraps the SDK with a non-secret local placeholder and immediately replaces the client key and HTTP headers with the real secret key. The secret is never logged or stored in source.
